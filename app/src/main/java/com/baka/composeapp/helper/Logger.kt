    ...
    private fun trace(): Array<String>? {
        ...
        for (i in stackTraceElements.indices) {
            ...
            if (ste.className == Logger::class.java.name && ste.methodName.contains(
                ...
                val containStartLogMethod = index < stackTraceElements.size &&
                        stackTraceElements[index].methodName.contains(START_LOG_METHOD)

                val containEndLogMethod = index < stackTraceElements.size &&
                        stackTraceElements[index].methodName.contains(END_LOG_METHOD)

                if (containStartLogMethod || containEndLogMethod) {
                    break
                }
